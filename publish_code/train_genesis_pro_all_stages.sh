...
usage() {
  cat <<'USAGE'
Usage:
  bash train_genesis_pro_all_stages.sh [--visual] [--timestamp YYYYMMDD_HHMMSS] [--start-stage 1|2|3] [HYDRA_OVERRIDES...]
...
Examples:
  ...
  STAGE2_CHECKPOINT=logs/20260622_pro_stage2/model_1600.pt bash train_genesis_pro_all_stages.sh --start-stage 3
  bash train_genesis_pro_all_stages.sh --timestamp 20260620_120000

Environment overrides:
  PYTHON_BIN              Python executable used by HumanoidVerse. Default: python
  NUM_ENVS                Forwarded through train_genesis_pro_staged.sh.
  RUN_TIMESTAMP           Shared timestamp for logs/<timestamp>_pro_stage{1,2,3}.
  START_STAGE             1 runs all; 2 uses latest stage1 ckpt; 3 uses latest stage2 ckpt.
